...
//style paths
gulp.task('sass', function () {
  return gulp.src('scss/**/*.scss',  { style: 'nested' })
...
gulp.task('build', ['build:app', 'build:vendor', 'sass']);

/**
 * Minify built vendor.js
 */
gulp.task('build:vendor-min', ['apply-prod-env', 'build:vendor', 'sass'], function() {
...
gulp.task('build:app-min', ['apply-prod-env', 'build:app', 'lint', 'sass'], function() {
...
gulp.task('default', ['build:vendor-min', 'build:app-min', 'sass']);
